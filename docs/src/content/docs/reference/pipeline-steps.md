...
- When [`pr.draft: true`](/no-mistakes/reference/repo-config/#prdraft) is set, the PR/MR is opened as a draft (`gh pr create --draft` / `glab mr create --draft` / `az repos pr create --draft true`). This is applied only at creation, never on a later update, so a human marking the PR ready for review in the provider UI is never silently reverted by a follow-up pipeline run. Bitbucket Cloud has no draft-PR concept and ignores the setting.
