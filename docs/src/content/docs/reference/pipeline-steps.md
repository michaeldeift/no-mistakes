...
- PRs/MRs open as drafts by default (`gh pr create --draft` / `glab mr create --draft` / `az repos pr create --draft true`); set [`pr.draft: false`](/no-mistakes/reference/repo-config/#prdraft) in a repo's `.no-mistakes.yaml` to open ready-for-review instead. This is applied only at creation, never on a later update, so a human marking the PR ready for review in the provider UI is never silently reverted by a follow-up pipeline run. Bitbucket Cloud has no draft-PR concept and ignores the setting - PRs there always open ready-for-review.
